...
@login_required
def translate_text():
    trans_info = translate(request.form['text'], request.form['source_language'], request.form['dest_language'])
    return jsonify(
        {'text': trans_info['trans_result'][0]['dst']}
    )
...
def send_message(recipient):
    ...
    if form.validate_on_submit():
        ...
        db.session.add(msg)
        user.add_notification(name='unread_message_count', data=user.new_messages())
        db.session.commit()
...
def notifications():
    ...
    notifications = \
        current_user.notifications.filter(Notification.timestamp > since).order_by(Notification.timestamp.asc())
